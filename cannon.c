...
int main(int argc, char* argv[])
{
    ...
    if(ma != na)
    {
        fprintf(stderr, "ERROR: Matrix A's dimensions are not equal.\n");
        return 1;
    }
    ...
    if(mb != nb)
    {
        fprintf(stderr, "ERROR: Matrix B's dimensions are not equal.\n");
        return 2;
    }

    if(ma != mb)
    {
        fprintf(stderr, "ERROR: Matrix A & B are not compatible.\n");
        return 3;
    }

    if(ma % p_sqrt != 0)
    {
        fprintf(stderr, "ERROR: The matrix is not divisible by the square root of the number of processes.\n");
        return 4;
    }

    /* YOUR CODE: THE CANNON ALGORITHM STARTS HERE */

    /* the following only shows how to multiply the two submatrices */
    ...
    // cannons algorithm start

    // id's for the neighboring blocks
    int proc_left, proc_right;
    int proc_up, proc_down;
    // for asynch send & recv
    MPI_Status status;
    MPI_Request request[4];

    // initial shifting to start cannons algorithm
    MPI_Cart_shift(comm, 1, coord[0], &proc_right, &proc_left);
    MPI_Cart_shift(comm, 0, coord[1], &proc_down, &proc_up);

    MPI_Sendrecv_replace(*a, n*n, mpitype, proc_left, 0, proc_right, 0, comm, &status);
    MPI_Sendrecv_replace(*b, n*n, mpitype, proc_up, 0, proc_down, 0, comm, &status);

    datatype *abuff, *bbuff;
    abuff = (datatype*)malloc(n*n*sizeof(datatype));
    bbuff = (datatype*)malloc(n*n*sizeof(datatype));

    for(int i = 0; i < p_sqrt; i++)
    {
        MPI_Cart_shift(comm, 1, 1, &proc_left, &proc_right);
        MPI_Cart_shift(comm, 0, 1, &proc_up, &proc_down);
        my_matmul(0, 0, 0, 0, 0, 0, n, n, n, n, a, b, c);

        MPI_Isend(sa, n*n, mpitype, proc_left, 0, comm, &request[0]);
        MPI_Irecv(abuff, n*n, mpitype, proc_right, 0, comm, &request[2]);

        MPI_Isend(sb, n*n, mpitype, proc_up, 0, comm, &request[1]);
        MPI_Irecv(bbuff, n*n, mpitype, proc_down, 0, comm, &request[3]);

        MPI_Waitall(4, request, MPI_STATUS_IGNORE);

        memcpy(sa, abuff, n*n*sizeof(datatype));
        memcpy(sb, bbuff, n*n*sizeof(datatype));
